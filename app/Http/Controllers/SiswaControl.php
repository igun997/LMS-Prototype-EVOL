      ...
      if ($ujian->count() > 0) {
        ...
        foreach ($data->ujian_items as $key => $value) {
          if ($value->banksoal->jenis == "pg") {
            $si[] = ["id"=>$value->id,"tipe"=>$value->banksoal->jenis,"soal"=>$value->banksoal->soal,"pg_a"=>$value->banksoal->pg_a,"pg_b"=>$value->banksoal->pg_b,"pg_c"=>$value->banksoal->pg_c,"pg_d"=>$value->banksoal->pg_d,"pg_e"=>$value->banksoal->pg_e];
          }else {
            $si[] = ["id"=>$value->id,"tipe"=>$value->banksoal->jenis,"soal"=>$value->banksoal->soal];
          }
        }
